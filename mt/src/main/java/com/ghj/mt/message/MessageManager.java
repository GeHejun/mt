    ...
    class SendMessageTask implements Runnable {
        ...
        public void run() {
            if (message.getReceiveIDs().size() < 2) {
                //发送单聊消息;
                SocketChannel channel = UserManager.getInstance().getUserChannel(message.getReceiveIDs().get(0));
                if (channel != null && channel.isActive()) {
                    channel.writeAndFlush(message);
                }
            } else {
                //发送群聊消息;
                CopyOnWriteArrayList<Integer> userList = UserManager.getInstance().getUserListInGroup(message.getGroupID());
